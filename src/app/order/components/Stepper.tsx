...
import { cn } from '@/lib/utils'; // tiny helper (optional). If you don’t have it, replace cn(...) with the joined className string.
...
const STEPS: Step[] = [
  ...
  { key: 'account', label: 'Account setup', href: '/order/account' },
];
...
export function Stepper({ current = 1, linkifyPast = true }: StepperProps) {
  return (
          ...
          <ol className="relative z-[1] flex items-center gap-8">
            {STEPS.map((s, i) => {
              ...
              const Pill = (
                <div
                  className={cn(
                    'mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full ring-1',
                    isActive
              ...
              // allow links only to past steps (prevents skipping forward)
